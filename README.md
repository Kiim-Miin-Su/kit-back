...
# 가능한 항목 자동 설치까지 시도
bash ./scripts/setup-dev.sh --preset=compose --install
```
...
# 가능한 항목 자동 설치까지 시도
powershell -ExecutionPolicy Bypass -File .\scripts\setup-dev.ps1 -Preset compose -Install
```
...
```bash
bash ./scripts/setup-dev.sh --preset=local-node --install
```
...
node ./scripts/init-env.mjs --preset=local-node --dry-run
