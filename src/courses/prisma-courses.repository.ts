...
import { CoursesDatabase, StoredCourseRecord } from "./courses.types";
...
export class PrismaCoursesRepository implements CoursesRepository {
  ...
  async read(): Promise<CoursesDatabase> {
    ...
    const mappedCourses: StoredCourseRecord[] = courses.map((course) => ({
      id: course.courseId,
      slug: course.slug,
      title: course.title,
      ...
      enrollmentStatus: course.defaultEnrollmentStatus,
      ...
      learningPoints: Array.isArray(course.learningPoints)
        ...
        : [],
      curriculumPreview: course.curriculumPreviews.map((item) => ({
        id: item.previewId,
        title: item.title,
  ...
  async write(database: CoursesDatabase): Promise<void> {
    ...
    const instructorUserIdByName = new Map(
      instructors.map((instructor) => [instructor.name, instructor.userId]),
    );
